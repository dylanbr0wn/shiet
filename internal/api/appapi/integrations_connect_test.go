...
func TestIntegrationConnectRPCs(t *testing.T) {
	...
	if len(providers.Msg.Providers) != 3 {
	...
	_, err = client.GetIntegrationAuthStatus(context.Background(), connect.NewRequest(&appv1.GetIntegrationAuthStatusRequest{Provider: "bitbucket"}))
	if connect.CodeOf(err) != connect.CodeInvalidArgument {
		t.Fatalf("unknown provider code = %v", connect.CodeOf(err))
	}
	_, err = client.ConnectIntegration(context.Background(), connect.NewRequest(&appv1.ConnectIntegrationRequest{Provider: "bitbucket"}))
	if connect.CodeOf(err) != connect.CodeInvalidArgument {
		t.Fatalf("connect unknown provider code = %v", connect.CodeOf(err))
	}
...
func TestIntegrationConnectRPCsUnimplementedWithoutProviders(t *testing.T) {
	...
	providers, err := client.ListIntegrationProviders(context.Background(), connect.NewRequest(&appv1.ListIntegrationProvidersRequest{}))
	if err != nil || len(providers.Msg.Providers) != 3 {
